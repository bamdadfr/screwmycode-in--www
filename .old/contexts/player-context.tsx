...
export function PlayerContextProvider({children, initialData}: Props) {
  ...
  const [title, setTitle] = useState<Title>(initialData?.title ?? null);
  const [artwork, setArtwork] = useState<Artwork>(initialData?.image ?? null);
  const [url, setUrl] = useState<Url>(initialData?.audio ?? null);
  const [speed, setSpeed] = useState<Speed>(initialData?.speed ?? 1);
